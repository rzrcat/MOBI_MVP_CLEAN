 ...
 */
export function convertToMarkdown(response) {
 ...
 */
function convertDatabaseToMarkdown(database) {
    let markdown = "";
        ...
        Object.entries(database.properties).forEach(([key, prop]) => {
            ...
            switch (propType) {
                ...
                case "multi_select":
                    ...
                    details = `Options: ${options.map((o) => o.name).join(", ")}`;
                    break;
                case "relation":
        ...
        switch (item.object) {
            ...
            case "database":
                if (resultType === "database") {
                    // Simple display
                    const dbTitle = extractRichText(item.title || []) ||
                        "Untitled Database";
 ...
 */
function convertPropertiesToMarkdown(properties) {
    if (!properties)
        return "";
        ...
        switch (property.type) {
            ...
            case "files":
                propValue = (property.files || [])
                    .map((file) => `[${file.name || "Attachment"}](${file.file?.url || file.external?.url || "#"})`)
                    .join(", ");
                break;
            case "checkbox":
...
function renderBlock(block) {
    ...
    if (!blockContent && blockType !== "divider")
        return "";
    switch (blockType) {
        case "paragraph":
        ...
        case "table_row":
            ...
            return `| ${blockContent.cells
                .map((cell) => escapeTableCell(extractRichText(cell)))
                .join(" | ")} |`;
        case "template":
