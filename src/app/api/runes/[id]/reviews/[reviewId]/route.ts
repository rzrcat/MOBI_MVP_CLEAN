import { createRouteHandlerClient } from '@supabase/auth-helpers-nextjs';
import { cookies } from 'next/headers';
import { NextRequest, NextResponse } from 'next/server';

export async function PUT(
  request: NextRequest,
  { params }: { params: { id: string; reviewId: string } }
) {
  const supabase = createRouteHandlerClient({ cookies });
  const { reviewId } = params;

  // 로그인한 사용자 확인
  const { data: session } = await supabase.auth.getSession();
  if (!session?.session?.user) {
    return NextResponse.json(
      { error: 'Authentication required' },
      { status: 401 }
    );
  }

  try {
    const { content } = await request.json();
    const userId = session.session.user.id;

    if (!content || content.trim().length === 0) {
      return NextResponse.json(
        { error: '한줄평 내용이 필요합니다.' },
        { status: 400 }
      );
    }

    if (content.length > 100) {
      return NextResponse.json(
        { error: '한줄평은 100자 이내여야 합니다.' },
        { status: 400 }
      );
    }

    // 한줄평 소유자 확인 또는 관리자 확인
    const { data: review } = await supabase
      .from('rune_reviews')
      .select('user_id')
      .eq('id', reviewId)
      .single();

    if (!review) {
      return NextResponse.json(
        { error: '한줄평을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }

    // 관리자 권한 확인
    const { data: adminRoles } = await supabase
      .from('admin_roles')
      .select('*')
      .eq('user_id', userId);
    const isAdmin = adminRoles && adminRoles.length > 0;

    if (review.user_id !== userId && !isAdmin) {
      return NextResponse.json(
        { error: '이 한줄평을 수정할 권한이 없습니다.' },
        { status: 403 }
      );
    }

    // 한줄평 업데이트
    const { data: updatedReview, error } = await supabase
      .from('rune_reviews')
      .update({ content })
      .eq('id', reviewId)
      .select(
        `
        id, 
        content, 
        created_at,
        likes,
        dislikes,
        is_blinded,
        users:user_id (
          id,
          full_name,
          avatar_url
        )
      `
      )
      .single();

    if (error) {
      console.error('Error updating rune review:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    // 현재 사용자의 좋아요/싫어요 상태 확인
    const { data: userReaction } = await supabase
      .from('rune_review_reactions')
      .select('reaction_type')
      .eq('review_id', reviewId)
      .eq('user_id', userId)
      .maybeSingle();

    // users 타입 단언
    type User = { id: string; full_name?: string; avatar_url?: string };
    const users = updatedReview.users as User | User[] | undefined;

    return NextResponse.json({
      id: updatedReview.id,
      content: updatedReview.content,
      author: Array.isArray(users)
        ? {
            id: users[0]?.id,
            name: users[0]?.full_name || '익명 사용자',
            avatar: users[0]?.avatar_url,
          }
        : {
            id: users?.id,
            name: users?.full_name || '익명 사용자',
            avatar: users?.avatar_url,
          },
      createdAt: updatedReview.created_at,
      likes: updatedReview.likes || 0,
      dislikes: updatedReview.dislikes || 0,
      isLiked: userReaction?.reaction_type === 'like',
      isDisliked: userReaction?.reaction_type === 'dislike',
      isBlinded: updatedReview.is_blinded,
    });
  } catch (error) {
    console.error('Unexpected error in rune review PUT:', error);
    return NextResponse.json(
      { error: 'Server error occurred' },
      { status: 500 }
    );
  }
}

export async function DELETE(
  request: NextRequest,
  { params }: { params: { id: string; reviewId: string } }
) {
  const supabase = createRouteHandlerClient({ cookies });
  const { reviewId } = params;

  // 로그인한 사용자 확인
  const { data: session } = await supabase.auth.getSession();
  if (!session?.session?.user) {
    return NextResponse.json(
      { error: 'Authentication required' },
      { status: 401 }
    );
  }

  try {
    const userId = session.session.user.id;

    // 한줄평 소유자 확인 또는 관리자 확인
    const { data: review } = await supabase
      .from('rune_reviews')
      .select('user_id')
      .eq('id', reviewId)
      .single();

    if (!review) {
      return NextResponse.json(
        { error: '한줄평을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }

    // 관리자 권한 확인
    const { data: adminRoles } = await supabase
      .from('admin_roles')
      .select('*')
      .eq('user_id', userId);
    const isAdmin = adminRoles && adminRoles.length > 0;

    if (review.user_id !== userId && !isAdmin) {
      return NextResponse.json(
        { error: '이 한줄평을 삭제할 권한이 없습니다.' },
        { status: 403 }
      );
    }

    // 한줄평 삭제
    const { error } = await supabase
      .from('rune_reviews')
      .delete()
      .eq('id', reviewId);

    if (error) {
      console.error('Error deleting rune review:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Unexpected error in rune review DELETE:', error);
    return NextResponse.json(
      { error: 'Server error occurred' },
      { status: 500 }
    );
  }
}
