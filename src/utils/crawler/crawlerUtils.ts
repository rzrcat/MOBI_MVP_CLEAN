...
import axiosInstance, { APIResponse } from '@/shared/api/axiosInstance';
...
const CRAWLER_LOG_LEVEL = (process.env.CRAWLER_LOG_LEVEL || 'info') as LogLevel;
...
export function log(level: LogLevel, message: string, details?: unknown): void {
  ...
  switch (level) {
    ...
    case LogLevel.ERROR:
      ...
      // Slack 웹훅이 설정되어 있으면 Slack으로 알림 전송
      if (CRAWLER_SLACK_WEBHOOK && level === LogLevel.ERROR) {
        sendSlackAlert(message, details).catch((err) =>
...
export async function fetchHTML(
  ...
  try {
    ...
    if (retries > 0) {
      ...
      log(
        LogLevel.WARN,
        `Error fetching ${url}, retrying in ${Math.round(retryDelay)}ms (${retries} retries left)`,
        {
 ...
 */
export function parseHTML(html: string): ReturnType<typeof cheerioLoad> {
  try {
    return cheerioLoad(html);
  } catch (error) {
    log(LogLevel.ERROR, 'Failed to parse HTML', error);
    throw new Error(
...
export function handleError(
  ...
  const details = {
    ...
    ...(normalizedError.stack
      ...
      : {}),
    ...(normalizedError.cause ? { cause: String(normalizedError.cause) } : {}),
  };
  ...
  try {
    if (
      process.env.CRAWLER_SLACK_WEBHOOK &&
      normalizedError.message.includes('critical')
    ) {
 ...
 */
export function validateCrawledData<T = unknown>(
  data: T,
  expectedType: string,
  context: string,
  options: {
  ...
  if (data === null || data === undefined) {
    log(LogLevel.ERROR, `${context}: 데이터가 비어 있습니다.`, { data });
    return false;
  }
  ...
  if (Array.isArray(data) && expectedType === 'array') {
    if (data.length === 0) {
      log(LogLevel.WARN, `${context}: 빈 배열이 반환됨`, { data });
      return false;
    }
    ...
    if (minArrayLength > 0 && data.length < minArrayLength) {
      log(
        LogLevel.WARN,
        `${context}: 배열 길이가 너무 짧습니다. 필요: ${minArrayLength}, 실제: ${data.length}`,
        { data }
      );
      return false;
    }
    ...
    // 사용자 정의 검증 함수 실행
    if (customValidator && !customValidator(data as T)) {
      log(LogLevel.ERROR, `${context}: 사용자 정의 유효성 검사 실패`, { data });
      return false;
    }
  ...
  if (typeof data !== expectedType) {
    log(
      LogLevel.ERROR,
      `${context}: 유효하지 않은 데이터 타입, 필요: ${expectedType}, 실제: ${typeof data}`,
      { data }
    );
    return false;
  }
